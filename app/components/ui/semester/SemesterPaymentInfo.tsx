...
const SemesterPaymentInfo = ({ semester }: { semester: SemesterDetails }) => {
  const stripeAmount =
    Number(semester?.price_after_discount) === 0 ? Number(semester?.price) : Number(semester?.price_after_discount);

  const calculatedGatewayFees = stripeAmount * 0.05;
  const discount = Number(semester?.price) - Number(semester?.price_after_discount);
      ...
      <Card className="mb-6 min-h-[108px] h-fit">
        <CardContent className="flex items-center gap-[13px] p-[10px]">
          <div className="w-[114px] h-[88px] flex items-center justify-center shrink-0 bg-muted rounded-md">
            <Image
              src={semester?.image_url_ar || '/placeholder.svg'}
              width={96}
              height={40}
              alt={semester?.name_ar || 'Semester image'}
            />
          </div>
          <div className="flex flex-col gap-3">
            <div className="w-fit text-foreground font-sans text-base font-medium">
              {semester?.name_ar}
              {semester?.name_en ? ` - ${semester?.name_en}` : ''}
            </div>
          </div>
        </CardContent>
      </Card>
        ...
        <CardContent className="p-[15px]">
          ...
          <div className="flex flex-col gap-[13px] w-full">
            <div className="w-full flex flex-row flex-wrap justify-between items-start lg:items-center">
              ...
              <div className="text-muted-foreground font-sans text-base font-normal">${semester?.price}</div>
            </div>
...
export default SemesterPaymentInfo;
