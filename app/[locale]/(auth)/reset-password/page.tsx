...
const Page = () => {
  ...
  const passwordSchema = Yup.object().shape({
    newPassword: Yup.string().required(tValidation('PasswordRequired')).min(8, tValidation('PasswordMin')),
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await axiosInstance.post(`${process.env.NEXT_PUBLIC_BASE_URL}/auth/reset-password`, {
        token,
        newPassword,
      });
