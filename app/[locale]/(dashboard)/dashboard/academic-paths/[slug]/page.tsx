...
const AcademicPathPage = () => {
  ...
  const { pathDetails, isLoading, error } = useAcademicPathDetails(slug as string);
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <div className="md:col-span-2">
            <div className="flex items-center gap-4 mb-4 flex-wrap">
              <Image
                src={isRTL ? pathDetails.image_url_ar : pathDetails.image_url_en}
            ...
            <div className="w-full overflow-x-auto ">
              <Tabs defaultValue="information" className="mb-8">
                <TabsList className="w-full flex-nowrap bg-white">
                  ...
                  <TabsTrigger value="discussions" className="tabs-trigger" disabled>
                    <LockKeyhole size={15} />
                    {tTabs('discussions')}
                  </TabsTrigger>
                ...
                <TabsContent value="information" className="mt-6">
                  <h2 className="text-lg md:text-xl font-bold mb-4">
                    {isRTL ? pathDetails.description_ar : pathDetails.description_en}
                  </h2>

                  {(pathDetails.who_should_join_ar || pathDetails.who_should_join_en) && (
                    <>
                      <h2 className="text-lg md:text-xl font-bold mb-4">{tCourse('whoShouldJoin')}</h2>
                      <div className="prose max-w-none">
                        <p className="text-gray-700">
                          {isRTL ? pathDetails.who_should_join_ar : pathDetails.who_should_join_en}
                        </p>
                      </div>
                    </>
                  )}
                ...
                <TabsContent value="semesters">
                  <SemestersTab semesters={pathDetails.semesters} isRTL={isRTL} />
                </TabsContent>
              </Tabs>
            </div>
          </div>
          ...
          <div className="md:col-span-1 lg:relative w-[90%] mx-4 fixed bottom-4 left-0 ">
            <div className="bg-white p-4 md:p-6 rounded-lg my-4 shadow-sm md:sticky md:top-4">
                ...
                {pathDetails.price_after_discount && (
                  <div className="text-red-500 text-center text-sm">
                    {tCourse('discount', {
                      ...
                      discount: (
                        ((pathDetails.price - pathDetails.price_after_discount) / pathDetails.price) *
                        100
                      ).toFixed(0),
                    })}
                  </div>
              ...
              <Button className="w-full bg-blue-600 mb-4">{tCourse('enrollNow')}</Button>
                ...
                {pathDetails.languages.length > 0 && (
                  <div className="flex items-center gap-2">
                    ...
                    <span className="break-words">
                      {tCourse('availableIn')}: {pathDetails.languages.join(', ')}
                    </span>
                  </div>
                )}
